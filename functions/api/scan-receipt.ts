...
export async function onRequestPost(context: { request: Request; env: CloudflareEnv }) {
  ...
  try {
    ...
    if (!imageBase64) {
      return new Response(JSON.stringify({ error: "Missing image data in request" }), { status: 400, headers });
    }
    ...
    if (!parsedResult) {
      const errMsg = lastError?.error?.message || "Failed to process receipt with Gemini model on Cloudflare.";
