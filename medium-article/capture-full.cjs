...
(async () => {
  ...
  for (const [match, name] of [["tax","taxes-full"],["report","reports-full"],["expense","expense-full"]]) {
    await page.evaluate((m) => {
      const b = [...document.querySelectorAll("button")].find(x => x.textContent.trim().toLowerCase().startsWith(m));
      if (b) b.click();
    }, match);
    await new Promise(r => setTimeout(r, 3500));
    await page.screenshot({ path: path.join(__dirname, "assets", name + ".png"), fullPage: true });
