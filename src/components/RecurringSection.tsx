...
import { getRecurringRules, addRecurringRule, deleteRecurringRule, processRecurringRules, getCategories, addCategory } from "../lib/db";
  ...
  return (
    <div className="flex flex-col gap-6 sm:gap-8">
      ...
      <div className="bg-white dark:bg-[#1A1A1A] border border-[#1A1A1A] dark:border-[#383838] p-4 sm:p-6 md:p-8 relative shadow-[3px_3px_0px_#1A1A1A] sm:shadow-[6px_6px_0px_#1A1A1A] dark:shadow-[6px_6px_0px_#000]">
        ...
        <form onSubmit={handleAddRule} className="space-y-3 mt-2">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 sm:gap-4 items-end">
            ...
            <div className="sm:col-span-1 lg:col-span-3">
              <label className="block text-[10px] uppercase font-bold tracking-widest text-[#1A1A1A] dark:text-[#E0E0E0] mb-1">Label / Title</label>
              <input 
                type="text" 
                required 
            ...
            <div className="sm:col-span-1 lg:col-span-2">
              <label className="block text-[10px] uppercase font-bold tracking-widest text-[#1A1A1A] dark:text-[#E0E0E0] mb-1">Frequency</label>
              <select 
                value={frequency} 
                onChange={e => setFrequency(e.target.value as any)} 
                className="w-full px-3 py-2 border border-[#1A1A1A] dark:border-[#444] bg-[#FCFAF7] dark:bg-[#242424] text-[#1A1A1A] dark:text-[#F0ECE1] focus:outline-none focus:border-[2px] dark:focus:border-white font-mono text-xs uppercase"
              >
                <option value="monthly" className="bg-white dark:bg-[#1A1A1A] text-[#1A1A1A] dark:text-[#F0ECE1]">Monthly</option>
                <option value="quarterly" className="bg-white dark:bg-[#1A1A1A] text-[#1A1A1A] dark:text-[#F0ECE1]">Quarterly</option>
                <option value="yearly" className="bg-white dark:bg-[#1A1A1A] text-[#1A1A1A] dark:text-[#F0ECE1]">Yearly</option>
              </select>
            </div>
            <div className="sm:col-span-1 lg:col-span-2">
              <label className="block text-[10px] uppercase font-bold tracking-widest text-[#1A1A1A] dark:text-[#E0E0E0] mb-1">Next Due Date</label>
              <input 
                type="date" 
                required 
                value={nextDueDate} 
      ...
      <div className="bg-white dark:bg-[#1A1A1A] border border-[#1A1A1A] dark:border-[#383838] p-4 sm:p-6 md:p-8 shadow-[4px_4px_0px_#1A1A1A] sm:shadow-[8px_8px_0px_#1A1A1A] dark:shadow-[8px_8px_0px_#000]">
        ...
        {loading ? (
          <p className="text-xs font-mono uppercase tracking-widest text-[#666] dark:text-[#A0A0A0] py-4">Loading rules...</p>
        ) : rules.length === 0 ? (
          <div className="p-6 text-center border border-dashed border-[#1A1A1A] dark:border-[#444] bg-[#FCFAF7] dark:bg-[#242424]">
            <p className="text-xs font-mono uppercase tracking-widest text-[#666] dark:text-[#A0A0A0]">No recurring rules configured yet.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {rules.map(rule => (
              <div key={rule.id} className="border border-[#1A1A1A] dark:border-[#383838] p-3 sm:p-3.5 bg-[#FCFAF7] dark:bg-[#242424] flex flex-col sm:flex-row justify-between sm:items-center gap-2 shadow-[2px_2px_0px_#1A1A1A] dark:shadow-[2px_2px_0px_#000]">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-serif italic font-bold text-sm sm:text-base text-[#1A1A1A] dark:text-[#F0ECE1]">{rule.label || rule.category_id}</span>
                ...
                <div className="flex justify-between sm:justify-end items-center gap-3 border-t sm:border-t-0 border-gray-300 dark:border-[#383838] pt-2 sm:pt-0">
                  <span className="font-mono text-sm sm:text-base font-bold text-red-700 dark:text-rose-400">₹{rule.amount.toLocaleString()}</span>
                  <button 
