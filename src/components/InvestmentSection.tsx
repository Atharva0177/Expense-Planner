...
import { TrendingUp, PlusCircle, RefreshCw, Briefcase, ChevronRight, Layers, DollarSign, Tag } from "lucide-react";
...
export function InvestmentSection() {
  ...
  const handleAddAccount = async (e: React.FormEvent) => {
    ...
    await addInvestmentAccount({
      ...
      name: accName.trim(),
      type: accType,
      custom_type_description: accType === "other" ? (customAccType.trim() || "Other") : undefined,
  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-in fade-in duration-300">
      <div className="border-b-2 border-[#1A1A1A] dark:border-[#383838] pb-2">
        <h2 className="text-xl sm:text-2xl font-serif italic tracking-tight text-[#1A1A1A] dark:text-[#F0ECE1]">Household Portfolio</h2>
        <p className="text-[10px] uppercase font-mono tracking-widest text-[#666] dark:text-[#A0A0A0] mt-0.5">Asset allocations & net valuation</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-6">
        <div className="border border-[#1A1A1A] dark:border-[#383838] bg-white dark:bg-[#1A1A1A] p-4 sm:p-6 shadow-[3px_3px_0px_#1A1A1A] sm:shadow-[4px_4px_0px_#1A1A1A] dark:shadow-[4px_4px_0px_#000]">
          <p className="text-[9px] sm:text-[10px] uppercase font-bold tracking-widest text-[#555] dark:text-[#A0A0A0] mb-1">Total Invested</p>
          <p className="text-2xl sm:text-3xl font-mono font-bold text-[#1A1A1A] dark:text-[#F0ECE1]">₹{totalInvested.toLocaleString()}</p>
        </div>
        <div className="border border-[#1A1A1A] dark:border-[#383838] bg-white dark:bg-[#1A1A1A] p-4 sm:p-6 shadow-[3px_3px_0px_#1A1A1A] sm:shadow-[4px_4px_0px_#1A1A1A] dark:shadow-[4px_4px_0px_#000]">
          <p className="text-[9px] sm:text-[10px] uppercase font-bold tracking-widest text-[#555] dark:text-[#A0A0A0] mb-1">Current Value</p>
          <p className="text-2xl sm:text-3xl font-mono font-bold text-[#1A1A1A] dark:text-[#F0ECE1]">₹{totalCurrentValue.toLocaleString()}</p>
        </div>
        <div className="border border-[#1A1A1A] dark:border-[#383838] bg-white dark:bg-[#1A1A1A] p-4 sm:p-6 shadow-[3px_3px_0px_#1A1A1A] sm:shadow-[4px_4px_0px_#1A1A1A] dark:shadow-[4px_4px_0px_#000]">
          <p className="text-[9px] sm:text-[10px] uppercase font-bold tracking-widest text-[#555] dark:text-[#A0A0A0] mb-1">Absolute Return</p>
          <div className="flex items-baseline gap-2">
            <p className={`text-2xl sm:text-3xl font-mono font-bold ${absoluteReturn >= 0 ? 'text-green-700 dark:text-emerald-400' : 'text-red-700 dark:text-rose-400'}`}>
              ₹{Math.abs(absoluteReturn).toLocaleString()}
            </p>
            <span className={`text-[11px] font-mono font-bold ${absoluteReturn >= 0 ? 'text-green-700 dark:text-emerald-400' : 'text-red-700 dark:text-rose-400'}`}>
      ...
      <div className="border border-[#1A1A1A] dark:border-[#383838] bg-white dark:bg-[#1A1A1A] p-4 sm:p-6 shadow-[3px_3px_0px_#1A1A1A] sm:shadow-[4px_4px_0px_#1A1A1A] dark:shadow-[4px_4px_0px_#000]">
        ...
        <form onSubmit={handleAddAccount} className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 items-end">
            <div>
              <label className="block text-[10px] uppercase font-bold tracking-widest text-[#1A1A1A] dark:text-[#E0E0E0] mb-1">Account Name</label>
              <input 
                type="text" 
            ...
            <div>
              <label className="block text-[10px] uppercase font-bold tracking-widest text-[#1A1A1A] dark:text-[#E0E0E0] mb-1">Asset Class</label>
              <select 
                value={accType} 
              ...
              >
                <option value="mutual_fund" className="bg-white dark:bg-[#1A1A1A] text-[#1A1A1A] dark:text-[#F0ECE1]">Mutual Fund</option>
                <option value="stock" className="bg-white dark:bg-[#1A1A1A] text-[#1A1A1A] dark:text-[#F0ECE1]">Stock / Equity</option>
                <option value="fd" className="bg-white dark:bg-[#1A1A1A] text-[#1A1A1A] dark:text-[#F0ECE1]">Fixed Deposit (FD)</option>
                <option value="ppf" className="bg-white dark:bg-[#1A1A1A] text-[#1A1A1A] dark:text-[#F0ECE1]">PPF</option>
                <option value="nps" className="bg-white dark:bg-[#1A1A1A] text-[#1A1A1A] dark:text-[#F0ECE1]">NPS</option>
                <option value="gold" className="bg-white dark:bg-[#1A1A1A] text-[#1A1A1A] dark:text-[#F0ECE1]">Sovereign Gold / Gold</option>
                <option value="other" className="bg-white dark:bg-[#1A1A1A] text-amber-700 dark:text-amber-400 font-bold">+ Other (Custom Asset Class)...</option>
              </select>
            </div>
            <div>
              <label className="block text-[10px] uppercase font-bold tracking-widest text-[#1A1A1A] dark:text-[#E0E0E0] mb-1">Folio / Acc ID (Optional)</label>
              <input 
                type="text" 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 border border-[#1A1A1A] dark:border-[#383838] bg-white dark:bg-[#1A1A1A] p-4 sm:p-6 shadow-[3px_3px_0px_#1A1A1A] sm:shadow-[4px_4px_0px_#1A1A1A] dark:shadow-[4px_4px_0px_#000] flex flex-col">
          <h3 className="text-base sm:text-lg font-serif italic tracking-tight text-[#1A1A1A] dark:text-[#F0ECE1] mb-3 sm:mb-4 border-b border-[#1A1A1A] dark:border-[#383838] pb-2 flex items-center justify-between">
            ...
            <span className="text-[10px] font-mono text-[#666] dark:text-[#A0A0A0]">({accounts.length})</span>
          </h3>
          
          {loading ? (
            <p className="text-xs font-mono text-[#666] dark:text-[#A0A0A0] py-4">Loading accounts...</p>
          ) : accounts.length === 0 ? (
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      <div className={`text-[10px] font-mono mt-0.5 uppercase ${selectedAccount === acc.id ? 'text-gray-300 dark:text-gray-600' : 'text-[#666] dark:text-[#A0A0A0]'}`}>
                        {acc.custom_type_description || (acc.type === 'other' ? 'Other' : acc.type.replace('_', ' '))}
                      </div>
                    </div>
                    {accVal !== undefined && (
                      <div className="text-right">
                        <div className="font-mono text-xs font-bold">₹{accVal.toLocaleString()}</div>
                        <div className={`text-[9px] font-mono ${selectedAccount === acc.id ? 'text-gray-300 dark:text-gray-600' : 'text-[#666] dark:text-[#A0A0A0]'}`}>Current</div>
                      </div>
                    )}
                  </button>
                );
            ...
            return (
              <div className="space-y-6">
                <div className="border-b border-[#1A1A1A] dark:border-[#383838] pb-3 flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                  <div>
                    <h3 className="text-lg sm:text-xl font-serif italic tracking-tight text-[#1A1A1A] dark:text-[#F0ECE1]">{acc.name}</h3>
                    <p className="text-[10px] uppercase font-mono text-[#666] dark:text-[#A0A0A0]">
                      {acc.custom_type_description || (acc.type === 'other' ? 'Other' : acc.type.replace('_', ' '))} {acc.folio_number && `• Folio: ${acc.folio_number}`}
                    </p>
                  </div>
                  {latestVal && (
                    <div className="bg-[#FCFAF7] dark:bg-[#242424] border border-[#1A1A1A] dark:border-[#383838] px-3 py-1 self-start sm:self-auto">
                      <span className="text-[9px] uppercase font-mono text-[#666] dark:text-[#A0A0A0] block">Latest Value ({latestVal.date})</span>
                      <span className="font-mono font-bold text-sm text-[#1A1A1A] dark:text-[#F0ECE1]">₹{latestVal.total_value.toLocaleString()}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Add Holding Form */}
                  <form onSubmit={handleAddHolding} className="space-y-3 border border-[#1A1A1A] dark:border-[#383838] p-4 bg-[#FCFAF7] dark:bg-[#242424]">
                    ...
                    {isUnitBased ? (
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <label className="block text-[9px] uppercase font-bold tracking-widest text-[#1A1A1A] dark:text-[#E0E0E0] mb-1">Units</label>
                          <input 
                            type="number" 
                            step="any" 
                            value={holdingUnits || ''} 
                        ...
                        <div>
                          <label className="block text-[9px] uppercase font-bold tracking-widest text-[#1A1A1A] dark:text-[#E0E0E0] mb-1">Buy NAV/Price (₹)</label>
                          <input 
                            type="number" 
                            step="any" 
                            value={holdingPrice || ''} 
                      ...
                      <div>
                        <label className="block text-[9px] uppercase font-bold tracking-widest text-[#1A1A1A] dark:text-[#E0E0E0] mb-1">Invested Amount (₹)</label>
                        <input 
                          type="number" 
                          step="any" 
                          value={holdingAmount || ''} 
                    ...
                    <div>
                      <label className="block text-[9px] uppercase font-bold tracking-widest text-[#1A1A1A] dark:text-[#E0E0E0] mb-1">Purchase Date</label>
                      <input 
                        type="date" 
                        value={holdingDate} 
                  ...
                  {/* Add Valuation Form */}
                  <form onSubmit={handleAddValuation} className="space-y-3 border border-[#1A1A1A] dark:border-[#383838] p-4 bg-[#FCFAF7] dark:bg-[#242424]">
                    <h4 className="text-[10px] uppercase font-bold tracking-widest text-[#1A1A1A] dark:text-[#E0E0E0] flex items-center gap-1.5 border-b border-[#1A1A1A] dark:border-[#383838] pb-1">
                    ...
                    <div>
                      <label className="block text-[9px] uppercase font-bold tracking-widest text-[#1A1A1A] dark:text-[#E0E0E0] mb-1">Valuation Date</label>
                      <input 
                        type="date" 
                        value={valDate} 
                ...
                {/* Holdings Table */}
                <div>
                  <h4 className="text-[10px] uppercase font-bold tracking-widest text-[#1A1A1A] dark:text-[#E0E0E0] mb-2">Recorded Holdings ({accHoldings.length})</h4>
                  {accHoldings.length === 0 ? (
                    <p className="text-xs font-mono text-[#666] dark:text-[#A0A0A0] py-3 italic">No purchase transactions logged yet.</p>
                  ) : (
                    <div className="overflow-x-auto border border-[#1A1A1A] dark:border-[#383838]">
                      <table className="w-full text-left border-collapse min-w-[280px]">
                        ...
                        <tbody>
                          {accHoldings.map((h, i) => (
                            <tr key={h.id || i} className="border-b border-gray-200 dark:border-[#333] text-xs font-mono hover:bg-[#FCFAF7] dark:hover:bg-[#242424] text-[#1A1A1A] dark:text-[#F0ECE1]">
