        ...
        <div className="bg-white dark:bg-[#1A1A1A] border border-[#1A1A1A] dark:border-[#383838] p-4 sm:p-6 md:p-8 shadow-[4px_4px_0px_#1A1A1A] sm:shadow-[8px_8px_0px_#1A1A1A] dark:shadow-[8px_8px_0px_#000] flex flex-col min-h-[500px]">
          <div className="flex flex-col sm:flex-row justify-between sm:items-end mb-6 border-b-2 border-[#1A1A1A] dark:border-[#383838] pb-4 shrink-0 gap-3">
            <div>
              ...
              <h2 className="text-2xl sm:text-3xl font-serif font-bold tracking-tight text-[#1A1A1A] dark:text-[#F0ECE1]">
                Loan Schedule
              </h2>
              <p className="text-xs font-mono uppercase tracking-wider text-[#666] dark:text-[#A0A0A0] mt-1">
                ...
                {selectedLoan.tenure_months} mo
              </p>
            </div>
            <div className="bg-[#FCFAF7] dark:bg-[#242424] p-2 sm:p-0 border sm:border-0 border-[#1A1A1A] dark:border-[#444]">
              <p className="text-[10px] uppercase font-bold tracking-widest text-[#666] dark:text-[#A0A0A0] sm:text-right">
                Monthly EMI
              </p>
              <p className="text-2xl sm:text-3xl font-serif font-bold text-red-700 dark:text-rose-400 sm:text-right">
          ...
          <div className="flex-grow flex flex-col lg:flex-row gap-6 lg:gap-8">
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="text-xs font-mono">
                  {schedule.map((row, idx) => (
                    <tr
                      key={idx}
                      className={`border-b border-[#1A1A1A] dark:border-[#333] border-dotted hover:bg-[#FCFAF7] dark:hover:bg-[#242424] ${row.is_prepayment ? "bg-[#F0F5F2] dark:bg-emerald-950/40 text-[#2A4B3A] dark:text-emerald-400 font-bold" : "text-[#1A1A1A] dark:text-[#F0ECE1]"}`}
                    >
                      <td className="py-2 px-2">
                        {row.month_number}{" "}
                        {row.is_prepayment && (
                          <span className="text-[9px] bg-green-200 dark:bg-emerald-900 px-1 py-0.5 ml-1 text-green-900 dark:text-emerald-200 font-bold uppercase">
                            Prepay
                          </span>
                        )}
                      </td>
                      ...
                      <td className="py-2 px-2 text-right text-red-700 dark:text-rose-400">
                        ₹{row.interest_component.toLocaleString()}
                      </td>
            ...
            <div className="w-full lg:w-72 shrink-0 border border-[#1A1A1A] dark:border-[#383838] p-4 bg-[#FCFAF7] dark:bg-[#242424]">
              <p className="text-[10px] uppercase font-bold tracking-widest border-b border-[#1A1A1A] dark:border-[#383838] pb-1 mb-3 text-[#1A1A1A] dark:text-[#E0E0E0]">
                Add Prepayment
              </p>
              ...
              <p className="text-[9px] uppercase font-mono mt-3 text-[#666] dark:text-[#A0A0A0] leading-relaxed">
                Prepayments reduce the total tenure while keeping the EMI
                constant.
              </p>
            </div>
          </div>
        ...
        <>
          ...
          <div className="bg-white dark:bg-[#1A1A1A] border border-[#1A1A1A] dark:border-[#383838] p-4 sm:p-6 md:p-8 relative shadow-[3px_3px_0px_#1A1A1A] sm:shadow-[6px_6px_0px_#1A1A1A] dark:shadow-[6px_6px_0px_#000]">
            ...
            <form
              ...
              className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 sm:gap-4 items-end mt-2"
            >
            ...
            {loading ? (
              ...
              <div className="p-6 text-center border border-dashed border-[#1A1A1A] dark:border-[#444] bg-[#FCFAF7] dark:bg-[#242424]">
                <p className="text-xs font-mono uppercase tracking-widest text-[#666] dark:text-[#A0A0A0]">
                  No loans configured yet. Add your loan details above to
                  calculate EMI & amortization schedule.
                </p>
              </div>
            ) : (
              <div className="space-y-3 sm:space-y-4">
                {loans.map((loan) => (
                  <div
                    key={loan.id}
                    className="border border-[#1A1A1A] dark:border-[#383838] p-3.5 sm:p-4 bg-[#FCFAF7] dark:bg-[#242424] flex flex-col sm:flex-row justify-between sm:items-center gap-3 shadow-[2px_2px_0px_#1A1A1A] dark:shadow-[2px_2px_0px_#000]"
                  >
                    <div>
                      <p className="text-[10px] uppercase font-bold tracking-widest text-[#666] dark:text-[#A0A0A0] mb-0.5">
                        Principal Loan Amount
                      </p>
                      <p className="text-xl sm:text-2xl font-serif font-bold text-[#1A1A1A] dark:text-[#F0ECE1]">
                    ...
                    <div className="flex sm:flex-col justify-between sm:text-right items-center sm:items-end border-t sm:border-t-0 border-gray-300 dark:border-[#383838] pt-2 sm:pt-0">
                      <div>
                        ...
                        <p className="text-lg sm:text-xl font-serif font-bold text-red-700 dark:text-rose-400">
