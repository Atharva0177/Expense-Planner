...
export type ExportReportType = "pl_statement" | "category_summary" | "transaction_ledger" | "tax_filing";
export type PeriodOption = "current_month" | "last_3_months" | "last_6_months" | "financial_year" | "all_time";
...
export function ExportModal({ isOpen, onClose, defaultMonth }: ExportModalProps) {
  const { user, household } = useAuth();
  const [reportType, setReportType] = useState<ExportReportType>("pl_statement");
  ...
  const getMonthsForPeriod = (periodChoice: PeriodOption, baseMonth: string): string[] => {
    ...
    try {
      ...
      if (reportType === "pl_statement") {
        ...
        if (format === "pdf") exportCategorySummaryPDF(items, periodLabel, household?.name);
          ...
          const csvHeaders = ["Category", "Spent (INR)", "Budget Limit (INR)", "Variance (INR)", "% of Total", "Transaction Count"];
          const csvRows = items.map(i => [i.category, i.spent, i.budget, i.variance, `${i.percentOfTotal}%`, i.count]);
        ...
        if (format === "pdf") exportLedgerPDF(transactions, periodLabel, household?.name);
        else if (format === "excel") exportLedgerExcel(transactions, periodLabel, household?.name);
      ...
      setExportSuccess(`${reportType.replace(/_/g, " ").toUpperCase()} exported successfully in ${format.toUpperCase()} format!`);
      setTimeout(() => setExportSuccess(null), 4000);
    } catch (error) {
      console.error("Export error:", error);
      alert("Error generating export. Please ensure you have data for the chosen period.");
    } finally {
        ...
        <div className="flex items-start justify-between border-b-2 border-[#1A1A1A] dark:border-[#383838] pb-3">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl sm:text-2xl font-serif font-bold text-[#1A1A1A] dark:text-[#F0ECE1]">Export Financial Reports</h2>
            </div>
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-serif font-bold text-xs sm:text-sm text-[#1A1A1A] dark:text-[#F0ECE1]">{rt.title}</span>
                    </div>
                  ...
                  <p className="text-[10px] text-[#555] dark:text-[#999] leading-relaxed line-clamp-2">{rt.desc}</p>
                </button>
              );
            })}
          </div>
        ...
        <div>
          ...
          {period !== "all_time" && (
            <div className="mt-2.5 flex items-center gap-2">
              <span className="text-[10px] font-mono uppercase text-[#666] dark:text-[#999]">Reference Month:</span>
              <input
                type="month"
                value={customMonth}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            >
              ...
              <div className="text-left">
                <span className="block text-xs uppercase font-bold tracking-wider">PDF Report</span>
                <span className="block text-[8px] opacity-80">Official Formatted Doc</span>
              </div>
            </button>
            ...
            >
              ...
              <div className="text-left">
                <span className="block text-xs uppercase font-bold tracking-wider">Excel (.XLSX)</span>
                <span className="block text-[8px] text-[#666] dark:text-[#AAA]">Multi-Sheet Workbook</span>
              </div>
            </button>
            ...
            >
              ...
              <div className="text-left">
                <span className="block text-xs uppercase font-bold tracking-wider">CSV Data</span>
                <span className="block text-[8px] text-[#666] dark:text-[#AAA]">Universal Spreadsheet</span>
              </div>
            </button>
          </div>
