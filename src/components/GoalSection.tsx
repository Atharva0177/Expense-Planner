        ...
        {loading ? (
          ...
          <div className="p-6 text-center border border-dashed border-[#1A1A1A] dark:border-[#444] bg-[#FCFAF7] dark:bg-[#242424]">
            <p className="text-xs font-mono uppercase tracking-widest text-[#666] dark:text-[#A0A0A0]">
              No goals configured yet. Set a financial milestone above to start
              tracking.
            </p>
          </div>
        ) : (
          <div className="space-y-6 sm:space-y-8">
            {goals.map((goal) => {
              const rule = recurringRules.find(
                (r) => r.id === goal.linked_recurring_rule_id,
              );
              ...
              return (
                <div
                  key={goal.id}
                  className="border border-[#1A1A1A] dark:border-[#383838] p-3.5 sm:p-4 bg-[#FCFAF7] dark:bg-[#242424] shadow-[2px_2px_0px_#1A1A1A] dark:shadow-[2px_2px_0px_#000]"
                >
                  <div className="flex flex-col sm:flex-row justify-between sm:items-baseline mb-2 gap-1">
                    <div className="flex items-center justify-between sm:justify-start gap-2">
                      <span className="font-serif italic text-lg sm:text-xl font-bold text-[#1A1A1A] dark:text-[#F0ECE1]">
                        {goal.name}
                      </span>
                      {isCompleted && (
                    ...
                    <div className="flex items-baseline justify-between sm:justify-end gap-2 text-right">
                      <span className="font-bold font-mono text-base sm:text-lg text-green-800 dark:text-emerald-400">
                        ₹{goal.current_amount.toLocaleString()}
                      </span>
                      <span className="text-xs text-[#666] dark:text-[#A0A0A0] font-mono">
                  ...
                  <div className="flex flex-col sm:flex-row justify-between text-[10px] font-mono uppercase tracking-wider text-[#666] dark:text-[#A0A0A0] gap-2 pt-1 border-t border-dotted border-gray-300 dark:border-[#383838]">
                    <div className="space-y-0.5">
                      ...
                      {rule && (
                        <p className="text-[#2A4B3A] dark:text-emerald-400 font-bold">
                          Auto-saving ₹{rule.amount.toLocaleString()}/
                          {rule.frequency}
                        </p>
                      )}
                      {projection && projection !== "Completed" && (
                        <p className="text-[#1A1A1A] dark:text-[#F0ECE1]">
                    ...
                    <div className="self-start sm:self-auto mt-1 sm:mt-0">
                      {contributeGoalId === goal.id ? (
                        <form
                          onSubmit={handleManualContribute}
                        ...
                        >
                          ...
                          <button
                            type="submit"
                            className="bg-[#1A1A1A] dark:bg-white text-white dark:text-[#121212] px-2.5 py-1 text-[10px] font-bold uppercase hover:opacity-80 touch-manipulation"
                          >
                            Add
                          </button>
                          <button
                            ...
                            className="border border-[#1A1A1A] dark:border-[#444] bg-white dark:bg-[#1A1A1A] text-[#1A1A1A] dark:text-[#F0ECE1] px-2 py-1 text-[10px] font-bold uppercase hover:bg-gray-100 dark:hover:bg-[#242424] touch-manipulation"
                          >
                            Cancel
                          </button>
                        </form>
