...
export function OverviewSection({ currentMonth }: { currentMonth: string }) {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        const [txData, incData, budData, goalData, loanData] =
          await Promise.all([
  ...
  if (loading) {
    return (
      <div className="text-[10px] uppercase font-bold tracking-widest text-[#1A1A1A] dark:text-[#A0A0A0] animate-pulse">
        Loading Overview...
      </div>
    );
  }
  ...
  const totalIncome = incomeEntries.reduce(
    (sum, entry) => sum + (entry.net_credited || 0),
    0,
  );
    ...
    <div className="space-y-4 sm:space-y-6 md:space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b-2 border-[#1A1A1A] dark:border-[#383838] pb-2">
        ...
        <label className="flex items-center gap-2 cursor-pointer text-[10px] uppercase font-bold tracking-widest text-[#1A1A1A] dark:text-[#C0C0C0] py-1">
          <input
            type="checkbox"
            checked={showMemberBreakdown}
            onChange={(e) => setShowMemberBreakdown(e.target.checked)}
            className="accent-[#1A1A1A] dark:accent-[#FFF] w-3.5 h-3.5"
          />
      ...
      {showMemberBreakdown && members.length > 0 && (
        <div className="border border-[#1A1A1A] dark:border-[#383838] bg-white dark:bg-[#1A1A1A] p-4 sm:p-6 shadow-[3px_3px_0px_#1A1A1A] sm:shadow-[4px_4px_0px_#1A1A1A] dark:shadow-[4px_4px_0px_#000]">
          <h3 className="text-base sm:text-lg font-serif italic tracking-tight text-[#1A1A1A] dark:text-[#F0ECE1] mb-3 border-b border-[#1A1A1A] dark:border-[#383838] pb-2">
            Member Breakdown
          </h3>
          ...
          <div className="block sm:hidden space-y-3">
            {members.map((m) => {
              const mInc = incomeEntries
                .filter((i) => i.user_id === m.user_id)
                .reduce((sum, i) => sum + (i.net_credited || 0), 0);
              ...
              return (
                <div
                  key={m.id}
                  className="border border-dashed border-[#1A1A1A] dark:border-[#444] p-3 bg-[#FCFAF7] dark:bg-[#242424]"
                >
                  <div className="flex justify-between items-start mb-1.5">
                    <span className="font-mono text-xs font-bold truncate max-w-[180px] text-[#1A1A1A] dark:text-[#F0ECE1]">
                      {m.email}
                    </span>
                    <span className="text-[9px] uppercase px-1.5 py-0.5 bg-black dark:bg-white text-white dark:text-[#121212] font-mono">
                      {m.role}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-xs font-mono pt-1">
                    <div>
                      <span className="text-[9px] text-[#666] dark:text-[#999] block uppercase">
                        Income
                      </span>
                    ...
                    <div>
                      ...
                      <span className="text-red-700 dark:text-rose-400 font-bold">
                        ₹{mExp.toLocaleString()}
                      </span>
                    </div>
                  </div>
              ...
              <tbody>
                {members.map((m) => {
                  ...
                  return (
                    <tr
                      key={m.id}
                      className="border-b border-[#1A1A1A] dark:border-[#333] border-dotted text-xs font-mono"
                    >
                      <td className="p-2 text-[#1A1A1A] dark:text-[#F0ECE1]">
                        {m.email}
                      </td>
                      <td className="p-2 uppercase text-[10px] text-[#555] dark:text-[#A0A0A0]">
                        {m.role}
                      </td>
                      <td className="p-2 text-green-700 dark:text-emerald-400 font-semibold">
                        ₹{mInc.toLocaleString()}
                      </td>
                      <td className="p-2 text-red-700 dark:text-rose-400 font-semibold">
                        ₹{mExp.toLocaleString()}
                      </td>
                    </tr>
                  );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-6">
        {/* Total Income */}
        <div className="border border-[#1A1A1A] dark:border-[#383838] bg-white dark:bg-[#1A1A1A] p-4 sm:p-6 shadow-[3px_3px_0px_#1A1A1A] sm:shadow-[4px_4px_0px_#1A1A1A] dark:shadow-[4px_4px_0px_#000]">
          <p className="text-[10px] uppercase font-bold tracking-widest text-[#555] dark:text-[#A0A0A0] mb-1 sm:mb-2">
            Total Income ({currentMonth})
          </p>
          <p className="text-2xl sm:text-3xl font-mono text-green-700 dark:text-emerald-400 font-bold">
        ...
        <div className="border border-[#1A1A1A] dark:border-[#383838] bg-white dark:bg-[#1A1A1A] p-4 sm:p-6 shadow-[3px_3px_0px_#1A1A1A] sm:shadow-[4px_4px_0px_#1A1A1A] dark:shadow-[4px_4px_0px_#000]">
          <p className="text-[10px] uppercase font-bold tracking-widest text-[#555] dark:text-[#A0A0A0] mb-1 sm:mb-2">
            Total Expenses ({currentMonth})
          </p>
          <p className="text-2xl sm:text-3xl font-mono text-red-700 dark:text-rose-400 font-bold">
        ...
        <div className="border border-[#1A1A1A] dark:border-[#383838] bg-white dark:bg-[#1A1A1A] p-4 sm:p-6 shadow-[3px_3px_0px_#1A1A1A] sm:shadow-[4px_4px_0px_#1A1A1A] dark:shadow-[4px_4px_0px_#000]">
          ...
          <p className="text-2xl sm:text-3xl font-mono text-[#1A1A1A] dark:text-[#F0ECE1] font-bold">
            ₹{savings.toLocaleString()}
          </p>
          <p className="text-[10px] uppercase font-bold tracking-widest text-[#555] dark:text-[#A0A0A0] mt-1 sm:mt-2">
            Rate: {savingsRate}%
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        {/* Budgets Summary */}
        <div className="border border-[#1A1A1A] dark:border-[#383838] bg-white dark:bg-[#1A1A1A] p-4 sm:p-6 shadow-[3px_3px_0px_#1A1A1A] sm:shadow-[4px_4px_0px_#1A1A1A] dark:shadow-[4px_4px_0px_#000]">
          <h3 className="text-base sm:text-lg font-serif italic tracking-tight text-[#1A1A1A] dark:text-[#F0ECE1] mb-3 sm:mb-4 border-b border-[#1A1A1A] dark:border-[#383838] pb-2">
            Top Budget Limits
          </h3>
          {budgets.length > 0 ? (
                  ...
                  <div key={b.id}>
                    <div className="flex justify-between items-center text-[10px] uppercase font-bold tracking-widest mb-1 gap-2 text-[#1A1A1A] dark:text-[#E0E0E0]">
                      ...
                      <span className="shrink-0 font-mono">
                        ...
                        {b.limit_amount.toLocaleString()} ({percent}%)
                      </span>
                    </div>
            ...
            <p className="text-[10px] uppercase font-bold tracking-widest text-[#555] dark:text-[#A0A0A0]">
                ...
                {loans.map((loan) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <span className="text-[9px] uppercase text-[#666] dark:text-[#999] block">
                          Principal
                        </span>
                        <span className="font-mono text-sm font-bold text-[#1A1A1A] dark:text-[#F0ECE1]">
                      ...
                      <div className="text-right">
                        ...
                        <span className="font-mono text-sm font-bold text-red-700 dark:text-rose-400">
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody>
                    {loans.map((loan) => (
                      <tr
                        key={loan.id}
                        className="border-b border-[#1A1A1A] dark:border-[#333] border-dotted text-xs font-mono hover:bg-gray-50 dark:hover:bg-[#242424]"
                      >
                        <td className="p-2 font-semibold text-[#1A1A1A] dark:text-[#F0ECE1]">
                          ₹{loan.principal.toLocaleString()}
                        </td>
                        <td className="p-2 text-[#555] dark:text-[#A0A0A0]">
                          {loan.interest_rate}%
                        </td>
                        <td className="p-2 text-red-700 dark:text-rose-400 font-semibold">
                          ₹{loan.emi_amount.toLocaleString()}
