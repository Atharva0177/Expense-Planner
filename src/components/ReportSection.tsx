...
export function ReportSection({ currentMonth }: { currentMonth: string }) {
  ...
  const [selectedPeriod, setSelectedPeriod] =
    useState<ReportPeriod>("current_month");
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const trendPromises = months.map(async (m) => {
        ...
        const income = (incs || []).reduce(
          (sum, i) => sum + (i.net_credited || 0),
          0,
        );
      ...
      const currentMonthData = trendResults.find(
  ...
  const handleExportPL = async (format: "pdf" | "excel" | "csv") => {
    ...
    try {
      ...
      setExportNotice(
        `Profit & Loss Statement (${format.toUpperCase()}) exported for ${periodLabel}`,
      );
  ...
  const handleExportCategorySummary = async (
    ...
    try {
      ...
      setExportNotice(
        `Category Summary (${format.toUpperCase()}) exported successfully!`,
      );
      setTimeout(() => setExportNotice(null), 4000);
    } catch (err) {
      console.error(err);
  ...
  const handleExportLedger = async (format: "pdf" | "excel" | "csv") => {
    ...
    try {
      ...
      if (format === "pdf") exportLedgerPDF(txs, periodLabel, household?.name);
        ...
        exportLedgerExcel(txs, periodLabel, household?.name);
      else if (format === "csv") exportLedgerCSV(txs, periodLabel);

      setExportNotice(
        `Transaction Ledger (${format.toUpperCase()}) exported with ${txs.length} records!`,
      );
      setTimeout(() => setExportNotice(null), 4000);
    } catch (err) {
      console.error(err);
  ...
  const handleExportTaxPack = async () => {
    ...
    try {
      ...
      for (let m = 4; m <= 12; m++)
        fyMonths.push(`${fyStart}-${m.toString().padStart(2, "0")}`);
      for (let m = 1; m <= 3; m++)
        fyMonths.push(`${fyStart + 1}-${m.toString().padStart(2, "0")}`);
      ...
      const matchingCalc =
        taxCalcs.find((t) => t.financial_year === fyStr) || taxCalcs[0] || null;
      ...
      setExportNotice(
        `Tax Filing Computation Pack (PDF) generated for FY ${fyStr}`,
      );
      setTimeout(() => setExportNotice(null), 4000);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="flex flex-col gap-6 sm:gap-8">
      ...
      <div className="bg-white dark:bg-[#1A1A1A] border-2 border-[#1A1A1A] dark:border-[#383838] p-5 sm:p-7 shadow-[6px_6px_0px_#1A1A1A] sm:shadow-[8px_8px_0px_#1A1A1A] dark:shadow-[8px_8px_0px_#000] flex flex-col gap-5 transition-colors">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b-2 border-[#1A1A1A] dark:border-[#383838] pb-4 gap-4">
          <div>
            ...
            <p className="text-[10px] uppercase font-mono tracking-widest text-[#666] dark:text-[#A0A0A0] mt-1">
              Export Official PDF Statements, Multi-Tab Excel Workbooks, and CSV
              Ledgers
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="border border-[#1A1A1A] dark:border-[#383838] bg-[#FCFAF7] dark:bg-[#202020] p-4 flex flex-col justify-between hover:shadow-[3px_3px_0px_#1A1A1A] dark:hover:shadow-[3px_3px_0px_#000] transition-all">
            <div>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <h3 className="font-serif font-bold text-sm text-[#1A1A1A] dark:text-[#F0ECE1]">
                    Profit & Loss
                  </h3>
                </div>
              ...
              <p className="text-[10px] text-[#555] dark:text-[#A0A0A0] leading-relaxed mb-4">
                Total gross revenues, payroll deductions (EPF, PT, TDS),
                operating expenses, and net surplus.
              </p>
            </div>
            <div className="flex flex-col gap-1.5 pt-2 border-t border-gray-300 dark:border-[#333]">
              ...
              >
                ...
                <span>
                  {exportLoading === "pl_pdf" ? "Exporting..." : "Download PDF"}
                </span>
              </button>
          ...
          <div className="border border-[#1A1A1A] dark:border-[#383838] bg-[#FCFAF7] dark:bg-[#202020] p-4 flex flex-col justify-between hover:shadow-[3px_3px_0px_#1A1A1A] dark:hover:shadow-[3px_3px_0px_#000] transition-all">
            <div>
              ...
              <p className="text-[10px] text-[#555] dark:text-[#A0A0A0] leading-relaxed mb-4">
                Detailed spend per category, budget limits, under/over variance,
                and % share of expenditures.
              </p>
            </div>
            <div className="flex flex-col gap-1.5 pt-2 border-t border-gray-300 dark:border-[#333]">
              <button
          ...
          <div className="border border-[#1A1A1A] dark:border-[#383838] bg-[#FCFAF7] dark:bg-[#202020] p-4 flex flex-col justify-between hover:shadow-[3px_3px_0px_#1A1A1A] dark:hover:shadow-[3px_3px_0px_#000] transition-all">
            <div>
              ...
              <p className="text-[10px] text-[#555] dark:text-[#A0A0A0] leading-relaxed mb-4">
                Granular itemized log with dates, modes (UPI/Card), merchants,
          ...
          <div className="border border-[#1A1A1A] dark:border-[#383838] bg-[#FCFAF7] dark:bg-[#202020] p-4 flex flex-col justify-between hover:shadow-[3px_3px_0px_#1A1A1A] dark:hover:shadow-[3px_3px_0px_#000] transition-all">
            <div>
              ...
              <p className="text-[10px] text-[#555] dark:text-[#A0A0A0] leading-relaxed mb-4">
                Standard deduction, Section 80C/80D, HRA exemption, home loan
                interest, and Old vs New regime.
              </p>
            </div>
            <div className="flex flex-col gap-1.5 pt-2 border-t border-gray-300 dark:border-[#333]">
              <button
        ...
        <div className="border-t border-[#1A1A1A] dark:border-[#383838] pt-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono font-bold text-[#1A1A1A] dark:text-[#F0ECE1]">
              Need to batch import bank statement transactions?
            </span>
          </div>
            ...
            <table className="w-full text-left border-collapse text-xs font-mono min-w-[300px] bg-white dark:bg-[#1E1E1E] text-[#1A1A1A] dark:text-[#F0ECE1]">
              ...
              <tbody>
                {importQueue.slice(0, 50).map((row, idx) => (
                  <tr
                    key={idx}
                    className="border-b border-dotted border-gray-300 dark:border-[#333] hover:bg-[#F0F5F2] dark:hover:bg-[#252525]"
                  >
                ...
                {importQueue.length > 50 && (
                  <tr>
                    <td
                      colSpan={3}
                      className="py-2 text-center text-[10px] italic text-[#666] dark:text-[#999]"
                    >
      ...
      {loading ? (
        <p className="text-xs font-mono uppercase tracking-widest text-[#666] dark:text-[#999] py-4">
          Loading Analytics & Visual Charts...
        </p>
      ) : (
            ...
            {pieData.length === 0 ? (
              ...
              <div className="h-52 sm:h-60 w-full relative">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  <span className="text-[9px] uppercase font-bold tracking-widest text-[#666] dark:text-[#A0A0A0]">
                    Total
                  </span>
                  <span className="font-serif italic font-bold text-sm sm:text-base text-[#1A1A1A] dark:text-[#F0ECE1]">
                    ₹
                    {trendData
                      .find((t) => t.month === currentMonth)
                ...
                <table className="w-full text-xs font-mono text-[#1A1A1A] dark:text-[#F0ECE1]">
                  <tbody>
                    {pieData.map((entry, index) => (
                      <tr
                        key={index}
                        className="border-b border-dotted border-gray-200 dark:border-[#333]"
                      >
                        <td className="py-1 flex items-center gap-2">
                          ...
                          ></div>
                          <span className="truncate max-w-[120px] sm:max-w-[140px] uppercase text-[10px]">
                            {entry.name}
                          </span>
                        </td>
                        <td className="py-1 text-right font-bold text-[11px]">
