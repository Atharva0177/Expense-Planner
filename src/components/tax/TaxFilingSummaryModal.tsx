...
export function TaxFilingSummaryModal({
  ...
  const chosenRegime =
    result.recommended_regime === "new" ? result.new_regime : result.old_regime;
      ...
      <div className="bg-[#FCFAF7] dark:bg-[#1C1C1C] border-2 border-[#1A1A1A] dark:border-[#383838] max-w-2xl w-full p-4 sm:p-6 shadow-[8px_8px_0px_#1A1A1A] dark:shadow-[8px_8px_0px_#000] space-y-4 max-h-[90vh] overflow-y-auto font-mono">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] uppercase font-mono tracking-widest text-[#777] dark:text-[#999]">
                FY {chosenRegime.financial_year} •{" "}
                {isNewRegime ? "Section 115BAC (New Regime)" : "Old Tax Regime"}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="space-y-4 text-xs">
          ...
          <div className="p-3 bg-white dark:bg-[#161616] border border-[#1A1A1A] dark:border-[#383838] grid grid-cols-2 gap-2 text-[11px]">
            <div>
              <span className="text-[#888] uppercase text-[9px] block">
                Assessee Identifier:
              </span>
              <span className="font-bold text-[#1A1A1A] dark:text-[#F0ECE1] truncate block">
                {userEmail || "Individual Taxpayer"}
              </span>
            </div>
            <div>
              <span className="text-[#888] uppercase text-[9px] block">
            ...
            <div className="space-y-1.5 text-[11px]">
              <div className="flex justify-between">
                ...
                <span className="font-bold">
                  ₹{chosenRegime.salary_income.toLocaleString("en-IN")}
                </span>
              </div>
              {chosenRegime.other_income > 0 && (
                <div className="flex justify-between">
                  ...
                  <span className="font-bold">
                    ₹{chosenRegime.other_income.toLocaleString("en-IN")}
                  </span>
                </div>
              )}
              {result.capital_gains_summary.total_gains > 0 && (
                <div className="flex justify-between">
                  ...
                  <span className="font-bold">
                    ₹
                    {result.capital_gains_summary.total_gains.toLocaleString(
                      "en-IN",
                    )}
                  </span>
                </div>
              ...
              <div className="flex justify-between font-bold border-t border-dotted border-[#1A1A1A] dark:border-[#444] pt-1 text-xs">
                ...
                <span>
                  ₹
                  {(
                    chosenRegime.gross_income +
                    result.capital_gains_summary.total_gains
          ...
          <div className="border border-[#1A1A1A] dark:border-[#383838] bg-white dark:bg-[#161616] p-3 space-y-2">
            ...
            <div className="space-y-1.5 text-[11px]">
              {Object.entries(chosenRegime.deduction_breakdown).map(
                ([label, val]) => (
                  <div
                    key={label}
                    className="flex justify-between text-[#555] dark:text-[#CCC]"
                  >
                    <span>{label}:</span>
                    <span className="font-bold">
              ...
              <div className="flex justify-between font-bold border-t border-dotted border-[#1A1A1A] dark:border-[#444] pt-1 text-xs text-[#2A4B3A] dark:text-emerald-400">
                ...
                <span>
                  - ₹{chosenRegime.total_deductions.toLocaleString("en-IN")}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="border border-[#1A1A1A] dark:border-[#383838] bg-white dark:bg-[#161616] p-3 space-y-2">
            ...
            <div className="space-y-1.5 text-[11px]">
              <div className="flex justify-between">
                ...
                <span className="font-bold">
                  ₹{chosenRegime.taxable_income.toLocaleString("en-IN")}
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span>
                  ₹
                  {(
                    chosenRegime.slab_tax + chosenRegime.rebate_87a
                  ).toLocaleString("en-IN")}
                </span>
              </div>
              {chosenRegime.rebate_87a > 0 && (
                <div className="flex justify-between text-emerald-600 dark:text-emerald-400">
                  ...
                  <span>
                    - ₹{chosenRegime.rebate_87a.toLocaleString("en-IN")}
                  </span>
                </div>
              )}
              {chosenRegime.capital_gains_tax > 0 && (
                <div className="flex justify-between">
                  ...
                  <span>
                    + ₹{chosenRegime.capital_gains_tax.toLocaleString("en-IN")}
                  </span>
                </div>
              )}
              {chosenRegime.surcharge > 0 && (
                <div className="flex justify-between text-amber-700 dark:text-amber-400">
                  ...
                  <span>
                    + ₹
                    {(
                      chosenRegime.surcharge -
                      chosenRegime.marginal_relief_surcharge
                    ).toLocaleString("en-IN")}
                  </span>
                </div>
              )}
              ...
              <div className="flex justify-between font-bold border-t border-[#1A1A1A] dark:border-[#383838] pt-2 text-sm text-[#1A1A1A] dark:text-white">
                ...
                <span className="text-red-700 dark:text-rose-400">
                  ₹{chosenRegime.total_tax.toLocaleString("en-IN")}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="border border-[#1A1A1A] dark:border-[#383838] bg-white dark:bg-[#161616] p-3 space-y-2">
            ...
            <div className="space-y-1.5 text-[11px]">
              <div className="flex justify-between text-[#555] dark:text-[#CCC]">
                ...
                <span>
                  ₹
                  {result.net_payable_or_refund.tds_deducted.toLocaleString(
                    "en-IN",
                  )}
                </span>
              </div>
              <div className="flex justify-between text-[#555] dark:text-[#CCC]">
                ...
                <span>
                  ₹
                  {result.net_payable_or_refund.advance_tax_paid.toLocaleString(
                    "en-IN",
                  )}
                </span>
              </div>
              <div
                className={`flex justify-between font-bold border-t border-dotted border-[#1A1A1A] dark:border-[#444] pt-2 text-sm ${
                  result.net_payable_or_refund.is_refund
              ...
              >
                <span>
                  {result.net_payable_or_refund.is_refund
                    ? "Net Refund Claimable on ITR:"
                    : "Balance Tax Payable (Self Assessment):"}
                </span>
                <span>
                  ₹
                  {Math.abs(
                    result.net_payable_or_refund.balance,
                  ).toLocaleString("en-IN")}
                </span>
              </div>
            </div>
