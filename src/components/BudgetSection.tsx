...
import { getBudgets, setBudget, getTransactions, getCategories, addCategory, cloneBudgets } from "../lib/db";
  ...
  return (
    <div className="flex flex-col gap-6 sm:gap-8">
        ...
        <form onSubmit={handleSetBudget} className="space-y-3 mt-2">
          <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 sm:gap-4 items-end">
            ...
            <div className="sm:col-span-4">
              <label className="block text-[10px] uppercase font-bold tracking-widest text-[#1A1A1A] dark:text-[#E0E0E0] mb-1">Monthly Limit (₹)</label>
              <input 
                type="number" 
                required 
                min="0" 
      ...
      <div className="border border-[#1A1A1A] dark:border-[#383838] p-4 sm:p-6 md:p-8 bg-white dark:bg-[#1A1A1A] shadow-[4px_4px_0px_#1A1A1A] sm:shadow-[8px_8px_0px_#1A1A1A] dark:shadow-[8px_8px_0px_#000]">
        ...
        {loading ? (
          <p className="text-xs font-mono uppercase tracking-widest text-[#666] dark:text-[#A0A0A0] py-4">Loading budgets...</p>
          ...
          <div className="p-6 text-center border border-dashed border-[#1A1A1A] dark:border-[#444] bg-[#FCFAF7] dark:bg-[#242424]">
            <p className="text-xs font-mono uppercase tracking-widest text-[#666] dark:text-[#A0A0A0]">No budgets set for this month. Use the form above to add limits.</p>
          </div>
        ) : (
              ...
              return (
                <div key={item.category} className="space-y-1.5">
                  <div className="flex justify-between items-baseline gap-2">
                    <span className="font-serif italic text-base sm:text-lg font-semibold truncate text-[#1A1A1A] dark:text-[#F0ECE1]">{item.category}</span>
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <span className={`font-mono text-xs sm:text-sm font-bold ${isOver ? 'text-red-700 dark:text-rose-400' : 'text-[#1A1A1A] dark:text-[#F0ECE1]'}`}>
