...
export function exportPLStatementPDF(data: PLStatementData) {
  ...
  const totalPayrollDeductions = data.incomes.reduce((acc, i) => {
    return (
      acc + (i.epf_deduction || 0) + (i.professional_tax || 0) + (i.tds || 0)
    );
  }, 0);

  const totalNetIncome =
    data.incomes.reduce((acc, i) => acc + (i.net_credited || 0), 0) ||
    totalGrossIncome - totalPayrollDeductions;
  ...
  const savingsRate =
    totalNetIncome > 0
      ? ((netSurplus / totalNetIncome) * 100).toFixed(1)
      : "0.0";
  ...
  data.transactions.forEach((t) => {
    categoryMap.set(
      t.category_id,
      (categoryMap.get(t.category_id) || 0) + t.amount,
    );
  });
  ...
  doc.setFontSize(9);
  doc.text(
    `Account / Family: ${data.householdName || "Personal Account"} (${data.userName || "Primary User"})`,
    14,
    37,
  );
  ...
  const cards = [
    {
      ...
      value: formatINR(totalNetIncome),
      color: [240, 245, 242] as [number, number, number],
    },
    {
      title: "TOTAL EXPENDITURES",
      value: formatINR(totalExpenses),
      color: [253, 242, 242] as [number, number, number],
    },
    {
      ...
      color:
        netSurplus >= 0
          ? ([240, 245, 242] as [number, number, number])
          : ([253, 242, 242] as [number, number, number]),
    },
    {
      title: "SAVINGS RATE",
      value: `${savingsRate}%`,
  ...
  data.incomes.forEach((i) => {
    ...
    if (i.hra)
      incomeRows.push(["House Rent Allowance (HRA)", formatINR(i.hra)]);
    if (i.special_allowance)
      incomeRows.push([
        "Special & Other Allowances",
        formatINR(i.special_allowance),
      ]);
    if (i.bonus)
      incomeRows.push(["Performance Bonus & Incentives", formatINR(i.bonus)]);
    if (i.other)
      incomeRows.push(["Other Incomes / Credits", formatINR(i.other)]);
    if (i.epf_deduction)
      incomeRows.push([
        "Less: EPF / PF Deduction",
        `-${formatINR(i.epf_deduction)}`,
      ]);
    if (i.professional_tax)
      incomeRows.push([
        "Less: Professional Tax (PT)",
        `-${formatINR(i.professional_tax)}`,
      ]);
    if (i.tds) incomeRows.push(["Less: TDS Deducted", `-${formatINR(i.tds)}`]);
    incomeRows.push([
      { content: "Net Credited Income", styles: { fontStyle: "bold" } },
      {
        content: formatINR(i.net_credited || 0),
  ...
  expenseRows.push([
    {
      content: "TOTAL OPERATING EXPENDITURES",
      styles: { fontStyle: "bold" },
    } as any,
  ...
  autoTable(doc, {
    startY: currentY,
    head: [
      ["EXPENDITURES & OUTFLOW CATEGORY", "AMOUNT (INR)", "% OF EXPENSES"],
    ],
    body: expenseRows,
    theme: "plain",
    headStyles: {
  ...
  if (summaryY < doc.internal.pageSize.getHeight() - 35) {
    ...
    doc.setTextColor(26, 26, 26);
    doc.text(
      `FINANCIAL RESULT: NET ${netSurplus >= 0 ? "SURPLUS" : "DEFICIT"} OF ${formatINR(Math.abs(netSurplus))}`,
      18,
      summaryY + 8,
    );
    ...
    doc.text(
      `This statement accounts for ${data.transactions.length} verified transactions and ${data.incomes.length} revenue entries.`,
      18,
      summaryY + 14,
    );
    doc.text(
      "Official computer-generated statement from Personal & Family Finance Engine.",
      18,
      summaryY + 18,
    );
  }
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(
      `Page ${i} of ${pageCount} — Confidential Financial Report`,
      pageWidth / 2,
  ...
  doc.save(
    `Profit_Loss_Statement_${data.periodLabel.replace(/\s+/g, "_")}.pdf`,
  );
}
...
export function exportPLStatementExcel(data: PLStatementData) {
  ...
  const totalDeductions = data.incomes.reduce(
    (acc, i) =>
      (i.epf_deduction || 0) + (i.professional_tax || 0) + (i.tds || 0),
    0,
  );
  const totalNetIncome =
    data.incomes.reduce((acc, i) => acc + (i.net_credited || 0), 0) ||
  ...
  const summaryRows = [
    ...
    ...data.incomes
      .filter((i) => (i.hra || 0) > 0)
      .map((i) => ["HRA Allowance", i.hra]),
    ...data.incomes
      .filter((i) => (i.special_allowance || 0) > 0)
      .map((i) => ["Special Allowances", i.special_allowance]),
    ...data.incomes
      .filter((i) => (i.bonus || 0) > 0)
      .map((i) => ["Bonus & Incentives", i.bonus]),
    ...data.incomes
      .filter((i) => (i.other || 0) > 0)
      .map((i) => ["Other Income", i.other]),
    ["TOTAL GROSS REVENUE", totalGrossIncome],
    [],
    ["PAYROLL DEDUCTIONS", "AMOUNT (INR)"],
    ...data.incomes
      .filter((i) => (i.epf_deduction || 0) > 0)
      .map((i) => ["EPF / Provident Fund", i.epf_deduction]),
    ...data.incomes
      .filter((i) => (i.professional_tax || 0) > 0)
      .map((i) => ["Professional Tax", i.professional_tax]),
    ...data.incomes
      .filter((i) => (i.tds || 0) > 0)
      .map((i) => ["Tax Deducted at Source (TDS)", i.tds]),
    ...
    [
      "Savings Rate (%)",
      totalNetIncome > 0 ? (netSurplus / totalNetIncome) * 100 : 0,
    ],
  ];
  ...
  const catRows = [
    ...
    ...Array.from(categoryMap.entries())
      ...
      .map(([cat, info]) => [
        ...
        info.count,
        totalExpenses > 0
          ? Number(((info.spent / totalExpenses) * 100).toFixed(2))
          : 0,
      ]),
  ];
  ...
  XLSX.writeFile(
    wb,
    `Financial_Statement_${data.periodLabel.replace(/\s+/g, "_")}.xlsx`,
  );
}

export function exportPLStatementCSV(data: PLStatementData) {
  const totalNetIncome = data.incomes.reduce(
    (acc, i) => acc + (i.net_credited || 0),
    0,
  );
  const totalExpenses = data.transactions.reduce(
    (acc, t) => acc + (t.amount || 0),
...
export function computeCategorySummaries(
  ...
  budgets.forEach((b) => {
    budgetMap.set(
      b.category_id,
      (budgetMap.get(b.category_id) || 0) + b.limit_amount,
    );
  });
  ...
  return Array.from(categoryMap.entries())
    .map(([cat, info]) => {
      ...
      return {
        ...
        percentOfTotal:
          totalExpenses > 0
            ? Number(((info.spent / totalExpenses) * 100).toFixed(1))
            : 0,
        count: info.count,
      };
    })
...
export function exportCategorySummaryPDF(
  ...
  doc.setFontSize(8.5);
  doc.text(
    `Period: ${periodLabel} | Account: ${householdName || "Personal"}`,
    14,
    17,
  );
  doc.text(
  ...
  doc.setFont("helvetica", "bold");
  doc.text(
    `Total Spends: ${formatINR(totalSpent)}   |   Total Budget Limit: ${formatINR(totalBudget)}   |   Net Variance: ${formatINR(totalVariance)}`,
    14,
    30,
  );
  ...
  const tableBody = items.map((item) => [
    ...
    item.budget > 0
      ? item.variance >= 0
        ? `+${formatINR(item.variance)} (Under)`
        : `-${formatINR(Math.abs(item.variance))} (Over)`
      : "No Budget",
    `${item.percentOfTotal}%`,
    item.count.toString(),
  ]);
  ...
  tableBody.push([
    ...
    {
      content: totalBudget > 0 ? formatINR(totalBudget) : "—",
      styles: { fontStyle: "bold" },
    } as any,
    {
      content:
    ...
    { content: "100%", styles: { fontStyle: "bold" } } as any,
    {
      content: items.reduce((acc, i) => acc + i.count, 0).toString(),
      styles: { fontStyle: "bold" },
    } as any,
  ]);
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setTextColor(130, 130, 130);
    doc.text(
      `Page ${i} of ${pageCount} — ExpensePlanner Category Analysis`,
      pageWidth / 2,
      doc.internal.pageSize.getHeight() - 8,
      { align: "center" },
    );
...
export function exportCategorySummaryExcel(
  ...
  XLSX.writeFile(
    wb,
    `Category_Summary_${periodLabel.replace(/\s+/g, "_")}.xlsx`,
  );
}
...
export function exportLedgerPDF(
  ...
  doc.text(`Total Records: ${transactions.length}`, pageWidth - 14, 17, {
    align: "right",
  });
  ...
  tableBody.push([
    ...
    {
      content: `${transactions.length} Entries`,
      styles: { fontStyle: "bold" },
    } as any,
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setTextColor(130, 130, 130);
    doc.text(
      `Page ${i} of ${pageCount} — Verified Transaction Ledger`,
      pageWidth / 2,
      doc.internal.pageSize.getHeight() - 8,
      { align: "center" },
    );
...
export function exportLedgerExcel(
  ...
  XLSX.writeFile(
    wb,
    `Transaction_Ledger_${periodLabel.replace(/\s+/g, "_")}.xlsx`,
  );
}
...
export function exportTaxPackPDF(data: TaxReportData) {
  ...
  doc.setFontSize(8.5);
  doc.text(
    `Financial Year: ${data.financialYear} (Assessment Year: ${getAssessmentYear(data.financialYear)})`,
    14,
    18,
  );
  doc.text(`Taxpayer: ${data.userName || "Individual"}`, pageWidth - 14, 18, {
    align: "right",
  });
  ...
  const totalEPF = data.incomes.reduce(
    (acc, i) => acc + (i.epf_deduction || 0),
    0,
  );
  const totalPT = data.incomes.reduce(
    (acc, i) => acc + (i.professional_tax || 0),
    0,
  );
  ...
  const taxRows = [
    [
      "1. Gross Salary / Professional Income",
      formatINR(
        totalGrossSalary ||
          (tc as any)?.gross_salary ||
          (tc as any)?.gross_income ||
          0,
      ),
    ],
    [
      "2. Less: Standard Deduction (Section 16 ia)",
      `-${formatINR(standardDeduction)}`,
    ],
    ["3. Less: Professional Tax (Section 16 iii)", `-${formatINR(totalPT)}`],
    [
      "4. Less: House Rent Allowance Exemption (Sec 10 13A)",
      `-${formatINR(hraExemption)}`,
    ],
    [
      "5. Less: Section 80C Deductions (EPF, ELSS, PPF, Life Ins)",
      `-${formatINR(Math.min(150000, eightyC))}`,
    ],
    [
      "6. Less: Section 80D Health Insurance Premium",
      `-${formatINR(Math.min(75000, eightyD))}`,
    ],
    [
      "7. Less: Section 24(b) Home Loan Interest Exemption",
      `-${formatINR(Math.min(200000, homeLoanInterest))}`,
    ],
  ...
  if (tc) {
    taxRows.push([
      "9. Computed Tax Liability (Old Regime)",
      formatINR(tc.old_regime_tax),
    ]);
    taxRows.push([
      "10. Computed Tax Liability (New Regime)",
      formatINR(tc.new_regime_tax),
    ]);
    taxRows.push([
      {
        content: `Recommended Regime: ${tc.recommended_regime.toUpperCase()} REGIME`,
        styles: { fontStyle: "bold" },
      } as any,
      {
        content: `Tax: ${formatINR(tc.recommended_regime === "old" ? tc.old_regime_tax : tc.new_regime_tax)}`,
        styles: { fontStyle: "bold" },
      } as any,
    ]);
  }
